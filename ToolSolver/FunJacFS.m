...
for i=1:step % solve value by pose number.
    %% ----------------------------------------       Pose      -----------------------------------------------%%
    PoseState_ThisStep = Pose_state(3*i-2:3*i,1); % Pose of this step
...
%% This is the Jacobian part for Fourier series. Eq. (16) - Eq. (22). We use sparse matrix.
        ...
        hatxy = jhatxy + PoseState_ThisStep(1:2) - thiFeaXstate(1:2); % This is \hat{p}^{jk}_i -- Eq.(19)
        ...
        drdtj_x = hatxy(1,:)./rk; drdtj_y = hatxy(2,:)./rk;
        ...
        dBdpx = dBdtx; 
        dBdpy = dBdtx;
        dBdan = zeros(fsN+1, length(thetak)); 
        dBdbn = dBdan; % later we need to transpose it.
        % Here we go
        for nfs = 0:fsN
            ns = nfs.*sin(nfs.*thetak); % n * sin
            nc = nfs.*cos(nfs.*thetak); % n * cos
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            dBdan(nfs+1,:) = cos(nfs.*thetak); %|________Eq. (20)
            dBdbn(nfs+1,:) = sin(nfs.*thetak);  %|
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            dcosdtjx = ns.*tmpxy1x_test;          %|   
            dsindtjx =-nc.*tmpxy1x_test;          %|________Eq. (19)
        ...
        dBdCx = -dBdtx + drdtj_x;    dBdCx = dBdCx';
        dBdCy = -dBdty + drdtj_y;      dBdCy = dBdCy';
        dBdtx = dBdtx - drdtj_x;        dBdtx = dBdtx';
        dBdty = dBdty - drdtj_y;        dBdty = dBdty';
